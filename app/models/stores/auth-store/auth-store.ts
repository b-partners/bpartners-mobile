...
export const AuthStoreModel = types
  ...
  .actions(self => ({
    getToken: flow(function* (code) {
      const signInApi = new AuthApi(self.environment.api);
      try {
        const { accessToken, refreshToken } = env.isCi ? { accessToken: 'zIgy3X41CQI2T6U4Djtrcsg-m1SRBCtAUfne5qIewVQ.iD8ky0fLWM8QOmIkDO5CsQREDSvV7WF29nhZbiYRJNo', refreshToken: null } : yield signInApi.getToken(code);
