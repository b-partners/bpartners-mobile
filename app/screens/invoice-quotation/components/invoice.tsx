        ...
        <View style={INVOICE_CONTAINER_STYLE}>
            <TouchableOpacity style={INVOICE_STYLE}>
                <View style={{...ROW_STYLE, ...BOTTOM_MARGIN_STYLE}}>
                    ...
                    <Text text={printCurrencyToMajors(item.totalPriceWithVat)}
                          style={{...HEADER_TEXT_STYLE, ...TOTAL_PRICE_WITH_THAT_STYLE}}/>
                </View>

                <View style={{...ROW_STYLE, ...{flex: 1}}}>
                    <View
                        style={{...ROW_STYLE, ...CENTERED_ROW, ...{flex: 1, flexWrap: 'wrap'}, ...BODY_TEXT_CONTAINER}}>
                        <Text text={`#${props.item.ref}`} style={BODY_TEXT_STYLE}/>
                        <View style={DATE_CONTAINER}>
                            <BulletSeparator style={BULLET_SEPARATOR_STYLE}
                                             containerStyle={BULLET_SEPARATOR_CONTAINER_STYLE}/>
                            <Text text={datePipe(props.item.sendingDate).split(' ')[0]}
                                  style={[BODY_TEXT_STYLE, DATE_TEXT_STYLE]}/>
                        </View>
                    </View>
