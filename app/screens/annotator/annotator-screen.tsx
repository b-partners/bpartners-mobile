...
import { AnnotatorLabelList } from './components';
import { RenderDistances, RenderPoints } from './styles';
...
export const AnnotatorScreen: FC<DrawerScreenProps<TabNavigatorParamList, 'annotator'>> = observer(function AnnotatorScreen({ navigation }) {
        ...
        try {
          ...
          annotations.map(item => {
            ...
            item.polygon.points.map(pt => {
              points.push({ x: pt.x / xRatio, y: pt.y / yRatio });
              polygonPoints.push({ x: pt.x / xRatio, y: pt.y / yRatio });
            });
            annotationArray.push({ polygonPoints, label: item.labelName });
  ...
  const renderDistances = () => {
    ...
    for (let i = 0; i < currentPolygonPoints.length; i++) {
      ...
      distances.push(
        <Text key={`distance_${i}`} style={{ left: midX, top: midY, ...RenderDistances.text }}>
          {distance.toFixed(2)}
        </Text>
      );
    }
    ...
    return currentPolygonPoints.map((point, index) => {
      ...
      return (
        <Animated.View
          ...
          style={{ left: point.x - 10, top: point.y - 10, ...RenderPoints.animatedView }}
        >
          <View style={RenderPoints.view} />
        </Animated.View>
      );
    });
  };
      ...
      <ErrorBoundary catchErrors='always'>
        ...
        <View testID='AnnotatorScreen' style={{ ...FULL, backgroundColor: color.palette.white, position: 'relative' }}>
          ...
          <ScrollView style={{ flexDirection: 'column', flex: 1, marginTop: spacing[2] }} contentContainerStyle={{ alignItems: 'center' }}>
            {annotations.map(item => (
              <AnnotatorLabelList annotation={item} key={item.id} />
            ))}
          </ScrollView>
        </View>
      </ErrorBoundary>
