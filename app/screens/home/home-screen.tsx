...
export const HomeScreen: FC<DrawerScreenProps<NavigatorParamList, 'home'>> = observer(({ navigation }) => {
  const { transactionStore, legalFilesStore, authStore, invoiceStore, marketplaceStore, prospectStore, customerStore, productStore, draftStore } = useStores();
  ...
  useEffect(() => {
    ...
    invoiceStore.getQuotations({ status: InvoiceStatus.PROPOSAL, page: 1, pageSize: 10 });
