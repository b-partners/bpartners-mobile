...
CLIENT_ID=$(aws ssm get-parameters --names "/bpartners/mobile/env/CLIENT_ID" --with-decryption --output text --query "Parameters[*].Value")
CLIENT_SECRET=$(aws ssm get-parameters --names "/bpartners/mobile/env/CLIENT_SECRET" --with-decryption --output text --query "Parameters[*].Value")
